...
function normalizeAdAccountId(input?: string) {
  const id = input || defaultAdAccountId;
  if (!id) throw new Error("Informe adAccountId ou configure META_AD_ACCOUNT_ID. Exemplo: act_1234567890");
...
server.tool(
  ...
  "Lista contas de anúncio disponíveis para o token atual.",
  {
...
server.tool(
  "meta_get_campaigns",
  "Lista campanhas da conta de anúncio.",
  {
...
server.tool(
  "meta_get_spend_today",
  "Mostra gasto de hoje da conta de anúncio.",
  {
    adAccountId: z.string().optional().describe("ID da conta. Ex: act_123 ou 123. Se vazio usa META_AD_ACCOUNT_ID."),
  },
  async ({ adAccountId }) => {
...
server.tool(
  "meta_get_insights",
  "Busca insights de campanhas, conjuntos, anúncios ou conta.",
  {
    adAccountId: z.string().optional().describe("ID da conta. Ex: act_123 ou 123. Se vazio usa META_AD_ACCOUNT_ID."),
...
server.tool(
  "meta_diagnose_delivery",
  "Diagnóstico read-only simples: campanhas ativas e métricas dos últimos 7 dias.",
  {
    adAccountId: z.string().optional().describe("ID da conta. Ex: act_123 ou 123. Se vazio usa META_AD_ACCOUNT_ID."),
  },
  async ({ adAccountId }) => {
